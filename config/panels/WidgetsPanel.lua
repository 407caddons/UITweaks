...
function addonTable.ConfigSetup.Widgets(panel, tab, configWindow)
    ...
    -- Session Stats: static time only (no cycling)
    local sessionStaticBtn = CreateFrame("CheckButton", "UIThingsWidgetsSessionStaticCheck", panel, "ChatConfigCheckButtonTemplate")
    sessionStaticBtn:SetPoint("TOPLEFT", 20, yOffset - 100)
    _G[sessionStaticBtn:GetName() .. "Text"]:SetText("Session Widget: show time only (no cycling)")
    sessionStaticBtn:SetChecked(UIThingsDB.widgets.sessionStaticOnly)
    sessionStaticBtn:SetScript("OnClick", function(self)
        UIThingsDB.widgets.sessionStaticOnly = self:GetChecked()
        UpdateWidgets()
    end)
end
