...
function addonTable.ConfigSetup.CompactGroupFinder(panel, tab)
    ...
    heightLabel:SetPoint("TOPLEFT", 20, -128)
    ...
    heightSlider:SetPoint("TOPLEFT", 20, -152)
    ...
    Helpers.CreateSectionHeader(panel, "Fields shown in each result", -207)
    Checkbox("LunaUITweaksCompactLFGActivity", "Dungeon / activity name", -236, "showActivity")
    Checkbox("LunaUITweaksCompactLFGPlaystyle", "Playstyle", -264, "showPlaystyle")
    Checkbox("LunaUITweaksCompactLFGVoice", "Voice-chat indicator", -292, "showVoiceChat")
    Checkbox("LunaUITweaksCompactLFGRating", "Leader Mythic+ rating", -320, "showRating")
    Checkbox("LunaUITweaksCompactLFGMembers", "Member count", -348, "showMemberCount")
    Checkbox("LunaUITweaksCompactLFGRoles", "Role data", -376, "showRoleData")

    local note = panel:CreateFontString(nil, "OVERLAY", "GameFontDisableSmall")
    note:SetPoint("TOPLEFT", 20, -416)
